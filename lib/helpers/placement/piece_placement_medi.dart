...
class TwoStartingPiecePlacement {
  ...
  TwoStartingPiecePlacement(this.allPieces);

  List<ChockABlockPiece> selectAndPlaceTwoInitialPieces() {
  ...
  ChockABlockPiece _selectAndPlacePiece(List<List<bool>> currentBoardState) {
    ...
    for (ChockABlockPiece selectedPiece in availablePieces) {
      ...
      // Create a temporary copy of the board state for testing
  ...
  BoardPosition? findValidPositionWithExistingPieces(
    ...
    while (attempts < MAX_ATTEMPTS) {
      ...
      // Check for small isolated regions
      if (causesSmallIsolatedRegions(tempBoardState)) {
        attempts++;
        continue;
      }

      // Specific checks for problematic pieces
  ...
  ChockABlockPiece selectFallbackPiece(List<List<bool>> currentBoardState) {
    ...
    // If all pieces are used (shouldn't happen with just 2), take the first one
    if (smallest == null) {
      smallest = allPieces.firstWhere((p) => !p.isStartingPiece, orElse: () => allPieces.first);
    }

    // Try to place it in a safe position
    ...
    // Last resort - use (0,0) or another corner
    List<BoardPosition> corners = [
      BoardPosition(0, 0),
      BoardPosition(0, BOARD_COLS - 2),
      BoardPosition(BOARD_ROWS - 2, 0),
      BoardPosition(BOARD_ROWS - 2, BOARD_COLS - 2)
    ];

    for (var pos in corners) {
      if (doesPieceFitOnBoard(smallest, pos) &&
          !doesOverlapWithExistingPieces(smallest, pos, currentBoardState)) {
    ...
    // Absolute last resort - force it at (0,0) regardless
  ...
  // Check for small isolated regions
  bool causesSmallIsolatedRegions(List<List<bool>> boardState) {
